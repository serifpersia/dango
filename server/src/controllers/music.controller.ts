...
import { parseUmpResponse, UmpCarry } from '../lib/ump.js'
...
async function readLibrary(yt: Awaited<ReturnType<typeof getAuthedInnertube>> & {}) {
  const lib = (await yt.music.getLibrary()) as unknown as {
  ...
  logger.info(
    { tracks: tracks.length, playlists: playlists.length, gridItems: gridItems.length },
    '[music] library parsed'
  )
  return { tracks, playlists }
}

interface PlaybackReporter {
  addToWatchHistory: () => Promise<unknown>
  updateWatchTime: (currentTime: number) => Promise<unknown>
}
...
const UMP_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:156.0) Gecko/20100101 Firefox/156.0'
...
async function decipherAudio(videoId: string): Promise<DecipheredAudio> {
  ...
  const reporter = info as unknown as PlaybackReporter
  for (const fmt of sorted) {
    try {
      ...
      if (url && url.startsWith('http')) {
        ...
        const approxMs =
          (fmt as { approx_duration_ms?: unknown }).approx_duration_ms ??
          (info as unknown as { basic_info?: { duration?: unknown } }).basic_info?.duration
        const durationSec =
          typeof approxMs === 'number'
            ? Math.max(Math.round(approxMs / 1000), 1)
            : bitrate > 0
              ? 240
              : 0
        const entry: DecipheredAudio = {
          url,
          mimeType: fmt.mime_type,
          fetchedAt: Date.now(),
          durationSec,
          reporter:
            typeof reporter.addToWatchHistory === 'function' &&
            typeof reporter.updateWatchTime === 'function'
              ? reporter
              : null,
          historySent: false,
          lastPingSec: -1,
...
async function reportPlayback(videoId: string, entry: DecipheredAudio, rangeStart: number) {
  const reporter = entry.reporter
  if (!reporter) return
  const run = async () => {
    const total = audioTotalCache.get(videoId)
    if (!entry.historySent) {
      entry.historySent = true
      entry.lastPingSec = 0
      await reporter.addToWatchHistory().catch(() => {})
      await reporter.updateWatchTime(0).catch(() => {})
      return
    }
    if (!total || !entry.durationSec) return
    const posSec = Math.floor((rangeStart / total) * entry.durationSec)
    if (posSec - entry.lastPingSec >= 15) {
      entry.lastPingSec = posSec
      await reporter.updateWatchTime(posSec).catch(() => {})
    }
  }
  await Promise.race([run(), new Promise((r) => setTimeout(r, 4000))])
}
...
function boundWindow(
  ...
  const size = small || start === 0 ? FIRST_CHUNK : AUDIO_CHUNK
  return clamp(start, start + size - 1)
}
...
function getUmpSession(key: string): UmpSession {
  let session = umpSessions.get(key)
  if (!session) {
    session = { cpn: randomCpn(), rn: 0, carry: null }
    umpSessions.set(key, session)
  }
  return session
}
...
export class MusicController {
    ...
    try {
      ...
      const streaming = (
        info as unknown as {
          ...
          basic_info?: { title?: string; author?: string }
        }
      ).streaming_data
      const formats = streaming?.adaptive_formats ?? []
      const audioOnly = formats.filter((f) => f.has_audio && !f.has_video)
      const sorted = [...audioOnly].sort((a, b) => (b.bitrate ?? 0) - (a.bitrate ?? 0))
      let streamUrl = ''
      let mimeType: string | undefined
      for (const fmt of sorted) {
        try {
          const url = await fmt.decipher((yt.session as unknown as { player?: unknown }).player)
          if (url && url.startsWith('http')) {
            streamUrl = url
            mimeType = fmt.mime_type
            break
      ...
      if (!streamUrl) return res.status(502).json({ error: 'No stream available' })
      const proxied = `/api/proxy?url=${encodeURIComponent(streamUrl)}&referer=${encodeURIComponent('https://music.youtube.com/')}`
      const basic = (info as unknown as { basic_info?: { title?: string; author?: string } })
        .basic_info
      res.json({
        url: proxied,
        mimeType,
  ...
  audio = async (req: Request, res: Response) => {
    ...
    try {
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        const clenMatch = entry.url.match(/[?&]clen=(\d+)/)
        if (clenMatch && !audioTotalCache.has(videoId)) {
          audioTotalCache.set(videoId, parseInt(clenMatch[1], 10))
        }
        const [rangeStart, rangeEnd] = boundWindow(req.headers.range, videoId, attempt > 0)
        if (entry.lastStart >= 0 && rangeStart < entry.lastStart - AUDIO_CHUNK) {
          entry.lastPingSec = -30
        }
        ...
        try {
          const upstream = await fetchWithRetry(
            ...
            { retries: 1, timeoutMs: 30000, signal: aborter.signal }
          )
        ...
        if (parsed.status === 3) {
          logger.warn('[music] stream protection blocked %s, refreshing token', videoId)
          resetPoTokens(videoId)
          umpSessions.delete(videoId)
          continue
        }
        ...
        const total = audioTotalCache.get(videoId)
        res.status(206)
        res.set('Content-Type', entry.mimeType || 'audio/webm')
        res.set('Content-Length', String(parsed.media.length))
        if (total) {
          res.set(
            'Content-Range',
  ...
  playlist = async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error({ err }, '[music] playlist failed')
      res.json({ tracks: [] })
    }
  }
